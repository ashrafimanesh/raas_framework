<?php
/**
 * Description of PersianDate
 *
 * @author ramin ashrafimanesh <ashrafimanesh@gmail.com>
 */
class PersianDate {
        public static $persian_month_names=array(
	'01'=>'&#1601;&#1585;&#1608;&#1585;&#1583;&#1740;&#1606;',
	'02'=>'&#1575;&#1585;&#1583;&#1740;&#1576;&#1607;&#1588;&#1578;',
	'03'=>'&#1582;&#1585;&#1583;&#1575;&#1583;',
	'04'=>'&#1578;&#1740;&#1585;',
	...
	public static  $persian_day_names=array(
	'6'=>'&#1588;&#1606;&#1576;&#1607;',
	'0'=>'&#1740;&#1705;&#1588;&#1606;&#1576;&#1607;',
	'1'=>'&#1583;&#1608;&#1588;&#1606;&#1576;&#1607;',
	'2'=>'&#1587;&#1607; &#1588;&#1606;&#1576;&#1607;',
	...
	/**
         * 
         * @param type $g_y 2016 or 2016-01-21
         * @param type $g_m
         * @param type $g_d
         * @return type
         */
	public static function gregorian_to_persian($g_y, $g_m='', $g_d='')
	{
                if(func_num_args()==1){
                    list($g_y,$g_m,$g_d)=explode('-',$g_y);
                }
		...
		return func_num_args()==1 ?  implode('-',array($jy,$jm, $jd)) : array($jy,$jm, $jd);
	}
	
	public static function persian_to_gregorian($j_y, $j_m='', $j_d='')
	{
                if(func_num_args()==1){
                    list($j_y,$j_m,$j_d)=explode('-',$j_y);
                }
		$g_days_in_month = array(31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
		$j_days_in_month = array(31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29);
		$jy = $j_y-979;
		...
		return func_num_args()==1 ? implode('-',array($gy,$gm,$gd)) : array($gy,$gm,$gd);
	}
        
        public static function persian_month($j_date){
            $j_date=explode('-',$j_date);
            return self::$persian_month_names[$j_date[1]];
        }
        
        public static function persian_month_date($j_date){
            $date=  explode(" ", $j_date);
            $j_date=explode('-',$date[0]);
            return $j_date[2].' '.self::$persian_month_names[$j_date[1]].' '.$j_date[0].' '.(isset($date[1]) ? $date[1] : '');
        }


        private static function to_date($g_date,$input)
	{
		...
		$persian_date=self::gregorian_to_persian($g_year,$g_month,$g_day);
		if($input=='Y') return $persian_date[0];
		if($input=='y') return substr($persian_date[0],-2);
		if($input=='M') return self::$persian_month_names[$persian_date[1]];
		if($input=='m') return $persian_date[1];
		if($input=='D') return self::$persian_day_names[date('w')];
		...
		if($input=='compelete') 
		{
			$persian_d=$persian_date[2];
			if($persian_d{0}=='0') $persian_d=substr($persian_d,1);
				return self::$persian_day_names[date('w')].' '.$persian_d.' '.self::$persian_month_names[$persian_date[1]].' '.$persian_date[0]; 
		}
	}
	
	private static function date($input)
	{
		return self::to_date(date('Y').date('m').date('d'),$input);		
	}
	
	private static function date_to($j_date)
	{
		...
		$gregorian_date=self::persian_to_gregorian($j_year,$j_month,$j_day);
		return $gregorian_date[0].'-'.$gregorian_date[1].'-'.$gregorian_date[2];
	}
	
	private static function sec_to_day($sec)
	{
